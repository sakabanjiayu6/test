...
test_1_valid_maze_loading() {
    ...
    result=$(python maze.py "$valid_maze" 2>&1)
...
test_2_invalid_file_path() {
    ...
    result=$(python maze.py "$invalid_maze" 2>&1)
...
test_3_maze_too_small() {
    ...
    result=$(python maze.py "$small_maze" 2>&1)
...
test_4_maze_too_large() {
    ...
    result=$(python maze.py "$large_maze" 2>&1)
    if [[ $result != *"The maze size does not meet the requirements"* ]]; then
...
test_5_no_start_position() {
    ...
    result=$(python maze.py "$no_start_maze" 2>&1)
...
test_6_no_exit_position() {
    ...
    result=$(python maze.py "$no_exit_maze" 2>&1)
...
test_9_win_condition() {
    ...
    result=$(echo -e "d\nd\nd\n" | python maze.py "$simple_win_maze" 2>&1)
